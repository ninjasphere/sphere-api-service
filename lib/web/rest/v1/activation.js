...
router.get('/node/:nodeId/activate', function(req, res) {
	...
	req.app.get('service').facet('activationService').then(function(activationService) {
		return activationService.call('activation.nodeAwaitingActivation', req.params.nodeId.toString()).timeout(25000);
...
router.post('/node', auth.requireUser({api: true}), function(req, res) {
	...
	req.app.get('service').facet('activationService').then(function(activationService) {
		return activationService.call('activation.userClaimingNode', req.user, req.body.nodeId.toString()).timeout(25000);
	}).done(function(result) {
		res.json(result);
	}, function(err) {
		res.send(err.code, err);
...
router.delete('/node/:nodeId', auth.requireUser({api: true}), function(req, res) {
	...
	req.app.get('service').facet('activationService').then(function(activationService) {
		return activationService.call('activation.userDeactivateNode', req.user, req.params.nodeId.toString()).timeout(25000);
	}).done(function(result) {
		res.json(result);
	}, function(err) {
		res.send(err.code, err);
