...
router.get('/things', auth.requireAPIAuth(), function(req, res) {
  var response = when.all([
    ...
    ]).spread(function(things, devices, channels){
      return restUtils.combineThings.call(this, things, devices, channels)
...
router.get('/things/:thingId', auth.requireAPIAuth(), function(req, res) {
  var response = when.all([
    ...
    ]).spread(function(thing, devices, channels){
      return restUtils.combineThing.call(this, devices, channels, thing)
