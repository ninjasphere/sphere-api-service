  ...
  it('should add laststate', function () {
    ...
    return when.all([
      ...
      return lastState.getCachedLastState.call(this, redis, user, results).then(function(payload){
        expect(payload[0].device.channels[0].lastState).to.exist;
        expect(payload[0].device.channels[0].lastState.timestamp).to.exist;
        expect(payload[0].device.channels[0].lastState.payload.position).to.equal(239000);
      })
    });
  });
